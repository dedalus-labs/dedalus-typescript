...
export interface TerminalsWSClientOptions extends WS.ClientOptions {
  /**
   ...
   */
  reconnect?: TerminalsWSReconnectOptions | null;
...
export class TerminalsWS extends TerminalsEmitter {
  url: URL;
  socket: WS.WebSocket;
  ...
  constructor(
    ...
    this._sendQueue = new SendQueue<TerminalsAPI.TerminalClientEvent>(maxQueueSize);
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<TerminalsStreamMessage> {
    ...
    const onClose = (code: number, reason: string, unsent: TerminalsAPI.TerminalClientEvent[]) => {
  ...
  private _connect(): WS.WebSocket {
    ...
    socket.on('message', (wsEvent) => {
      const event = (() => {
        try {
          return JSON.parse(wsEvent.toString()) as TerminalsAPI.TerminalServerEvent;
        } catch (err) {
